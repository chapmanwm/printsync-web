...
export async function getPrints(filter?: { claimed?: boolean }) {
  if (filter?.claimed === false) {
    return await sql`SELECT * FROM prints WHERE claimed_by IS NULL ORDER BY created_at DESC`;
  } else if (filter?.claimed === true) {
    return await sql`SELECT * FROM prints WHERE claimed_by IS NOT NULL ORDER BY created_at DESC`;
  }

  return await sql`SELECT * FROM prints ORDER BY created_at DESC`;
}
